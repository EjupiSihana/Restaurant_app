...
class InvoiceBuilder:
    ...
    def __get_order_item_info(self, order_item):
        product = order_item.get_product()
        total_order_item_price = order_item.get_order_item_price() * order_item.get_quantety()
        ...
        order_item_info = (
            f"{order_item.get_quantety()} x| {product.get_product_id()}."
    ...
    def __get_order_info_header(self, client):
        order_header = (
            f"Order from{client.get_name()} /n"
            "----------------------------------"
